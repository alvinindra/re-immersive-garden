// Ported verbatim from the real site (default bundle shader s036).
// Compiled as GLSL3 (three provides texture2D->texture + varying->in, but not
// gl_FragColor) so the screen-space derivatives dFdx/dFdy are core.
#include ./config.glsl
...
vec4 srgbEncodeLocal(in vec4 value) {
...
void main() {
  ...
  vec3 bake1 = srgbEncodeLocal(texture2D(tBake1, vUv)).rgb;
  vec3 bake2 = srgbEncodeLocal(texture2D(tBake2, vUv)).rgb;
