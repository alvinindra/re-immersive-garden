...
export class Relief {
  ...
  private sweep: { active: boolean; t: number; dur: number; from: Vector2; to: Vector2; next: number } = {
    active: false,
    t: 0,
    dur: 1,
    from: new Vector2(),
  ...
  private loadTextures() {
    ...
    const maskNoise = raw(loader.load("/rgb-noise.jpg"), true); // bas-relief/noise
    const flowNoise = raw(loader.load("/mask-noise.png"), true); // flowmap tNoise
      ...
      gltf.load(
        ...
        (data) => {
          data.scene.traverse((child) => {
            ...
            if (tBake1) tBake1.colorSpace = LinearSRGBColorSpace;
            if (tBake2) tBake2.colorSpace = LinearSRGBColorSpace;
  ...
  /** Center the relief and pick the fov that fits its height at the configured
   *  distance, capped at config.camera.fov (= the real site's onConfigUpdated). */
  private frameModel() {
    const box = new Box3().setFromObject(this.model);
    const size = box.getSize(new Vector3());
    const center = box.getCenter(new Vector3());
    // re-center on origin
    this.model.children[0]?.position.sub(center);
  ...
  private updateCameraFov() {
    ...
    const fovToFit = 2 * Math.atan(this.modelHeight / (2 * dist)) * (180 / Math.PI);
    this.camera.fov = Math.min(CONFIG.camera.fov, fovToFit);
  ...
  private updateSweep(dt: number) {
    ...
    this.flowmap.mouse2.set(s.from.x + (s.to.x - s.from.x) * e, s.from.y + (s.to.y - s.from.y) * e);
    if (k >= 1) {
      s.active = false;
  ...
  update = () => {
    ...
    const flow = this.flowmap.texture;
    this.shared.tFlow.value = flow;
    this.shared.tFluidFlowmap.value = flow;
